...
def test_a_clean_draft_is_written_once_and_audited_once(profile, vcimco_jd, fake_claude):
    fake = fake_claude()
    written = run(profile, posting_from(vcimco_jd), fake)
...
def test_the_cost_of_every_call_is_summed(profile, vcimco_jd, fake_claude):
    written = run(profile, posting_from(vcimco_jd), fake_claude())
...
def test_a_draft_failing_the_lint_is_revised_before_anyone_pays_to_audit_it(
    ...
    written = run(profile, posting_from(vcimco_jd), fake, lint=lint)
...
def test_a_draft_that_runs_to_two_pages_is_sent_back_to_be_cut(profile, vcimco_jd, fake_claude):
    fake = fake_claude(packages=[fake_claude.package(), fake_claude.package()])
    pages = iter([2, 1])
    written = run(profile, posting_from(vcimco_jd), fake, fit=lambda p: next(pages))
    assert written.verified
    assert "2 pages" in fake.calls[1][2]


def test_a_draft_that_never_fits_produces_nothing(profile, vcimco_jd, fake_claude):
    written = run(profile, posting_from(vcimco_jd), fake_claude(), fit=lambda p: 2)
...
def test_an_unsupported_claim_is_sent_back_and_fixed(profile, vcimco_jd, fake_claude):
...
def test_a_wrong_authorization_statement_blocks(profile, vcimco_jd, fake_claude):
    fake = fake_claude(reviews=[fake_claude.review(verdict="revise",
                                                   authorization_correct=False)])
    assert run(profile, posting_from(vcimco_jd), fake).status == "refused"


def test_style_findings_are_recorded_but_do_not_block(profile, vcimco_jd, fake_claude):
...
def test_a_blocker_finding_forces_a_revision(profile, vcimco_jd, fake_claude):
    fake = fake_claude(reviews=[
        fake_claude.review(verdict="revise", findings=[{
            "severity": "blocker", "where": "p2", "problem": "wrong employer named",
            "suggestion": "name VCIMCO"}]),
        fake_claude.review(),
    ])
    written = run(profile, posting_from(vcimco_jd), fake)
...
def test_your_own_edits_are_audited_but_never_rewritten(profile, vcimco_jd, fake_claude):
    fake = fake_claude(reviews=[fake_claude.review(
...
def test_without_an_audit_a_passing_draft_is_labelled_unverified(profile, vcimco_jd,
                                                                 fake_claude):
    fake = fake_claude()
    written = run(profile, posting_from(vcimco_jd), fake,
...
def test_no_credential_is_reported_not_raised(profile, vcimco_jd):
    def unavailable(**_):
        raise llm.LLMUnavailable("no key")
    written = run(profile, posting_from(vcimco_jd), unavailable)
    assert written.status == "unavailable" and "no key" in written.reason


def test_the_budget_is_checked_before_every_call(profile, vcimco_jd, fake_claude, conn):
    budget = Budget(conn, caps={"per_call_usd": 0.0001, "per_run_usd": 5, "monthly_usd": 5})
    with pytest.raises(BudgetExceeded):
        run(profile, posting_from(vcimco_jd), fake_claude(), budget=budget)


def test_every_call_lands_in_the_ledger(profile, vcimco_jd, fake_claude, conn):
    budget = Budget(conn)
    fake = fake_claude(reviews=[
        fake_claude.review(verdict="revise", unsupported_claims=["x"]),
        fake_claude.review()])
    run(profile, posting_from(vcimco_jd), fake, budget=budget)
...
def test_the_writer_is_told_the_owner_s_default_proof_asset(profile, blackrock_jd):
    _, user = writer.prompts(profile, posting_from(blackrock_jd), [])
    assert "OhCamel" in user


def test_the_allocator_default_is_the_consulting_role_not_ohcamel(profile, vcimco_jd):
    _, user = writer.prompts(profile, posting_from(vcimco_jd), [])
    task = user.split("# TASK", 1)[1]
    assert "consulting" in task and "OhCamel" not in task


def test_the_writer_is_given_the_exact_authorization_statement(profile, vcimco_jd):
    _, user = writer.prompts(profile, posting_from(vcimco_jd), [])
    assert "Will not require employment sponsorship, now or in the future." in user


def test_source_material_is_first_and_cached_so_the_audit_reuses_it(profile, vcimco_jd):
    system, _ = writer.prompts(profile, posting_from(vcimco_jd), [])
...
def test_nothing_private_is_sent(profile, vcimco_jd, workspace):
    (workspace / "data" / "profile.private.yaml").write_text(
        "identity:\n  phone: '+1 804 555 0123'\n  street_address: '1 Main St'\n"
        "education_private:\n  gpa: '3.91'\n")
    from apply.profile import Profile

    system, user = writer.prompts(Profile.load(), posting_from(vcimco_jd), [])
